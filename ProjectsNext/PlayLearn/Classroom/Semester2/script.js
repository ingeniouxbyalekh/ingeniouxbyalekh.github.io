document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadFragment = (fileName) => {
        ...
        fetch(fileName)
            ...
            .catch(() => {
                ...
                getTimetableFallback(monthNum, dayNum).then(renderContent);
            });
    };
    ...
    // empty on weekends. Async because subject tiles resolve their
    // click-through URL from Firebase's /links table before rendering.
    async function getTimetableFallback(month, day) {
        ...
        classesArray.forEach((cls, index) => {
            ...
            const entry = cls.subjectCode ? linksMap[`${cls.subjectCode}-Notes`] : null;
            ...
            cardsHTML += `
                <div class="stat-card ${highlightClass} ${clickableClass}" ${clickableAttrs}>
                    <div class="stat-text">
        ...
        return `
            <style>
                ...
                .stat-card.clickable:hover { box-shadow: 0 4px 14px rgba(0,0,0,0.12); }
    ...
    // Short-code -> Firebase subject-code map. These are the same codes
    // used in SUBJECTS (subjects.js); the /links table keys entries as
    // "<subjectCode>-Notes", so this is how a schedule tile finds its
    // notes link at click time instead of a hardcoded Drive URL.
    ...
    // Dataset Parser for Semester 2 Schedules — each class carries its
    // Firebase subject code so its tile can resolve a click-through link.
    ...
    // Cached fetch of Firebase's /links table (defined in subjects.js) so
    // switching between dates repeatedly doesn't re-hit the database.
    ...
    // --- 4.1 EVENT DELEGATION FOR SUBJECT TILES ---
    // Tiles are injected/replaced on every date change, so this listens
    // on the stable parent container rather than re-binding per render.
    if (fragmentContainer) {
        const openTileLink = (tile) => {
            const url = tile.getAttribute('data-url');
            if (url) window.open(url, '_blank', 'noopener,noreferrer');
        };

        fragmentContainer.addEventListener('click', (e) => {
            const tile = e.target.closest('.stat-card[data-url]');
            if (tile) openTileLink(tile);
        });

        // Keyboard accessibility for the same clickable tiles.
        fragmentContainer.addEventListener('keydown', (e) => {
