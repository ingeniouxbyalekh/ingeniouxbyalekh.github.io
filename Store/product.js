...
// products.js), so the lookup — and the "not found" redirect — has
// to wait until it's actually populated, rather than running the
// instant this script executes.
let PRODUCT = null;
let heroQty = 1;
...
function renderProductPage() {
  ...
  document.getElementById("page-title").textContent = `${p.title} — INGENIOUX Shop`;
  document.getElementById("page-description").setAttribute("content", `${p.blurb} ${p.format}, ₹${p.price}.`);

  document.getElementById("breadcrumb").innerHTML =
    `<a href="index.html">Shop</a> / <a href="index.html">${catLabel}</a> / ${p.title}`;
  ...
  document.getElementById("info-price").textContent = p.price.toLocaleString("en-IN");
  document.getElementById("info-format").textContent = p.format;
  document.getElementById("mobile-price").textContent = p.price.toLocaleString("en-IN");
  ...
  document.getElementById("about-block").innerHTML = `
    <p>${p.blurb}</p>
    <p>Part of the INGENIOUX ${catLabel.toLowerCase()} collection — independent research work, written and maintained the same way it's produced for the main site, just packaged to keep, print, or build on.</p>
  `;
...
function renderRelated(p) {
  const sameCategory = INGENIOUX_PRODUCTS.filter((x) => x.id !== p.id && x.category === p.category);
  const others = INGENIOUX_PRODUCTS.filter((x) => x.id !== p.id && x.category !== p.category);
...
/* ---------------------------------------------------------------
   Quantity stepper (hero) — separate from the cart drawer's
   per-line qty control, which lives in cart.js. For physical items,
   the "+" is capped at admin-set stock (products-physical/<id>.stock)
   rather than an arbitrary number — see productStock() in cart.js.
--------------------------------------------------------------- */
function wireQtyStepper() {
  const stepperEl = document.getElementById("hero-qty");
  if (!stepperEl) return;

  // Digital items are one-per-shopper — no stepper, quantity stays 1.
  ...
  // Out of stock — applyStockState() already hides/disables
  // everything else on the page; nothing to wire up here.
  const maxQty = typeof productStock === "function" ? productStock(PRODUCT) : Infinity;
  if (maxQty <= 0) return;

  const displayEl = stepperEl.querySelector("span");

  stepperEl.querySelector('[data-action="dec"]').addEventListener("click", () => {
    heroQty = Math.max(1, heroQty - 1);
    displayEl.textContent = heroQty;
  });
  stepperEl.querySelector('[data-action="inc"]').addEventListener("click", () => {
    if (heroQty >= maxQty) {
      if (typeof showStockLimitPopup === "function") showStockLimitPopup(PRODUCT);
      return;
    }
    heroQty = Math.min(maxQty, heroQty + 1);
    displayEl.textContent = heroQty;
  });
}

/* ---------------------------------------------------------------
   Out-of-stock state (physical items only) — hides the qty stepper
   and disables the add/buy controls (hero row + the mobile buy bar)
   in favor of a plain "Out of stock" label, mirroring the catalog
   card treatment in catalog.js.
--------------------------------------------------------------- */
function applyStockState() {
  if (!PRODUCT || PRODUCT.type !== "physical") return;
  if (typeof productStock !== "function" || productStock(PRODUCT) > 0) return;

  const stepperEl = document.getElementById("hero-qty");
  const addBtn = document.getElementById("hero-add-btn");
  const buyBtn = document.getElementById("hero-buy-btn");
  const mobileBuyBtn = document.querySelector(".mobile-buy-bar .hero-buy-btn");
  ...
  if (mobileBuyBtn) {
    ...
    mobileBuyBtn.textContent = "Out of stock";
  }
}

/* ---------------------------------------------------------------
   Hero add-to-cart / buy-now — override the generic cart.js
   delegated handler just for these two buttons, so the chosen
...
function wireHeroButtons() {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      // Digital items are one-per-shopper — this button toggles
      // add/remove rather than stacking quantity.
      if (PRODUCT.type === "digital" && cart[PRODUCT_ID]) {
        removeFromCart(PRODUCT_ID);
      } else {
        addToCart(PRODUCT_ID, heroQty);
      }
    });
  }
  if (buyBtn) {
    buyBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      openCheckout(PRODUCT_ID, heroQty);
    });
  }
}

// cart.js calls this after every render so the hero button label
// can reflect how many of this item are already in the cart. Runs on
// every cart change (not just page load), so it re-checks stock each
// time too — otherwise a later renderCart() would overwrite the "Out
// of stock" label applyStockState() set at load.
function syncProductPageButtons() {
  const addBtn = document.getElementById("hero-add-btn");
  if (!addBtn || typeof cart === "undefined") return;

  if (typeof isOutOfStock === "function" && isOutOfStock(PRODUCT)) {
    addBtn.disabled = true;
    addBtn.textContent = "Out of stock";
    return;
  }

  const qtyInCart = cart[PRODUCT_ID] || 0;
  if (PRODUCT && PRODUCT.type === "digital") {
    addBtn.textContent = qtyInCart > 0 ? "Remove from cart" : "Add to cart";
  } else {
    addBtn.textContent = qtyInCart > 0 ? `In cart · ${qtyInCart}` : "Add to cart";
  }
}
...
INGENIOUX_PRODUCTS_READY.then(() => {
  PRODUCT = typeof findProduct === "function" ? findProduct(PRODUCT_ID) : null;
